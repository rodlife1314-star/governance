...
export default function FieldBar({
  ...
  return (
    <div className="border-b border-white/[0.05] bg-[#07080B] shrink-0" id="field-bar">
      ...
      {/* ── Row 2: Domain selector (always visible) ── */}
      <div className="flex items-center px-4 md:px-6 pb-2 pt-0">
        <DomainSelector selected={selectedDomain} onSelect={onDomainChange} />
      </div>

      {/* ── Row 3: Asset selector — only when Finance active ── */}
      {selectedDomain === "FINANCE" && (
        <>
          {/* Mobile: asset pills + price */}
